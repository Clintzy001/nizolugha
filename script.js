document.addEventListener('DOMContentLoaded', () => {
    ...
    // Update XP Display (Profile Screen)
    // Note: Ensure your XP span has the class 'stat-val-xp' or id 'user-xp'
    ...
    navItems.forEach(item => {
        item.addEventListener('click', function() {
            // If the item is a link (<a>), the browser handles navigation.
            // If it's a div, we handle the active class:
    ...
    if (nextBtn) {
        nextBtn.addEventListener('click', (e) => {
            ...
            if (currentLanguageData && currentPhraseIndex < currentLanguageData.lessons.length - 1) {
                ...
                // 2. Update Streak (only once per day)
                ...
                // 4. Trigger Success Overlay (Defined in lesson.html)
